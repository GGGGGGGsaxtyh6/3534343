...
public final class ImmutableRangeSet<C extends Comparable> extends AbstractRangeSet<C> implements Serializable {
    ...
    private ImmutableList<Range<C>> intersectRanges(final Range<C> range) {
        ...
        if (zHasUpperBound) {
            size = SortedLists.binarySearch(immutableList, (Function<? super E, Cut<C>>) Range.lowerBoundFn(), range.upperBound, SortedLists.KeyPresentBehavior.FIRST_PRESENT, SortedLists.KeyAbsentBehavior.NEXT_HIGHER);
        } else {
    ...
    private final class AsSet extends ImmutableSortedSet<C> {
        ...
        int indexOf(@CheckForNull Object target) {
            ...
            while (it.hasNext()) {
                ...
                if (zContains) {
                    return Ints.saturatedCast(size + ((long) ContiguousSet.create(range, discreteDomain).indexOf(comparable)));
                }
