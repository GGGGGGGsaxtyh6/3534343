...
public class SyncEngine implements RemoteStore.RemoteStoreCallback {
    ...
    public int listen(QueryOrPipeline queryOrPipeline, boolean z) {
        assertCallback("listen");
        Assert.hardAssert(!this.queryViewsByQuery.containsKey(queryOrPipeline), "We already listen to query: %s", queryOrPipeline);
        TargetData targetDataAllocateTarget = this.localStore.allocateTarget(queryOrPipeline.toTargetOrPipeline());
        this.syncEngineListener.onViewSnapshots(Collections.singletonList(initializeViewAndComputeSnapshot(queryOrPipeline, targetDataAllocateTarget.getTargetId(), targetDataAllocateTarget.getResumeToken())));
        if (z) {
    ...
    private ViewSnapshot initializeViewAndComputeSnapshot(QueryOrPipeline queryOrPipeline, int i, ByteString byteString) {
        ...
        this.queryViewsByQuery.put(queryOrPipeline, new QueryView(queryOrPipeline, i, view));
        if (!this.queriesByTarget.containsKey(Integer.valueOf(i))) {
            this.queriesByTarget.put(Integer.valueOf(i), new ArrayList(1));
        }
        this.queriesByTarget.get(Integer.valueOf(i)).add(queryOrPipeline);
        return viewChangeApplyChanges.getSnapshot();
    }

    public void listenToRemoteStore(QueryOrPipeline queryOrPipeline) {
        assertCallback("listenToRemoteStore");
        Assert.hardAssert(this.queryViewsByQuery.containsKey(queryOrPipeline), "This is the first listen to query: %s", queryOrPipeline);
        this.remoteStore.listen(this.localStore.allocateTarget(queryOrPipeline.toTargetOrPipeline()));
    }

    void stopListening(QueryOrPipeline queryOrPipeline, boolean z) {
        assertCallback("stopListening");
        QueryView queryView = this.queryViewsByQuery.get(queryOrPipeline);
        Assert.hardAssert(queryView != null, "Trying to stop listening to a query not found", new Object[0]);
        this.queryViewsByQuery.remove(queryOrPipeline);
        int targetId = queryView.getTargetId();
        List<QueryOrPipeline> list = this.queriesByTarget.get(Integer.valueOf(targetId));
        list.remove(queryOrPipeline);
        if (list.isEmpty()) {
    ...
    void stopListeningToRemoteStore(QueryOrPipeline queryOrPipeline) {
    ...
    public void handleOnlineStateChange(OnlineState onlineState) {
        ...
        Iterator<Map.Entry<QueryOrPipeline, QueryView>> it = this.queryViewsByQuery.entrySet().iterator();
        while (it.hasNext()) {
    ...
    public ImmutableSortedSet<DocumentKey> getRemoteKeysForTarget(int i) {
        ...
        if (this.queriesByTarget.containsKey(Integer.valueOf(i))) {
            for (QueryOrPipeline queryOrPipeline : this.queriesByTarget.get(Integer.valueOf(i))) {
                if (this.queryViewsByQuery.containsKey(queryOrPipeline)) {
                    immutableSortedSetEmptyKeySet = immutableSortedSetEmptyKeySet.unionWith(this.queryViewsByQuery.get(queryOrPipeline).getView().getSyncedDocuments());
                }
            }
        }
    ...
    private void removeAndCleanupTarget(int i, Status status) {
        for (QueryOrPipeline queryOrPipeline : this.queriesByTarget.get(Integer.valueOf(i))) {
            this.queryViewsByQuery.remove(queryOrPipeline);
            if (!status.isOk()) {
                this.syncEngineListener.onError(queryOrPipeline, status);
                logErrorIfInteresting(status, "Listen for %s failed", queryOrPipeline);
            }
        }
    ...
    private void emitNewSnapsAndNotifyLocalStore(ImmutableSortedMap<DocumentKey, Document> immutableSortedMap, RemoteEvent remoteEvent) {
        ...
        while (it.hasNext()) {
            ...
            ViewChange viewChangeApplyChanges = value.getView().applyChanges(documentChangesComputeDocChanges, targetChange, z);
    ...
    private void pumpEnqueuedLimboResolutions() {
        while (!this.enqueuedLimboResolutions.isEmpty() && this.activeLimboTargetsByKey.size() < this.maxConcurrentLimboResolutions) {
            ...
            this.remoteStore.listen(new TargetData(new TargetOrPipeline.TargetWrapper(Query.atPath(next.getPath()).toTarget()), iNextId, -1L, QueryPurpose.LIMBO_RESOLUTION));
        }
    }
