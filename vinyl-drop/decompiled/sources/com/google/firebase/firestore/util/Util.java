...
public class Util {
    ...
    private static String getUtf8SafeBytes(String str, int i) {
        return str.substring(i, Character.charCount(str.codePointAt(i)) + i);
    }

    public static FirebaseFirestoreException exceptionFromStatus(Status status) {
        StatusException statusExceptionAsException = status.asException();
        return new FirebaseFirestoreException(statusExceptionAsException.getMessage(), FirebaseFirestoreException.Code.fromValue(status.getCode().value()), statusExceptionAsException);
    }
    ...
    public static String toDebugString(ByteString byteString) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            sb.append(Character.forDigit(bByteAt & Ascii.SI, 16));
        }
        return sb.toString();
    }
    ...
    public static int compareByteStrings(ByteString byteString, ByteString byteString2) {
        ...
        for (int i = 0; i < iMin; i++) {
            ...
            int iByteAt2 = byteString2.byteAt(i) & 255;
            if (iByteAt < iByteAt2) {
                return -1;
            }
            if (iByteAt > iByteAt2) {
