...
final class MemoryMutationQueue implements MutationQueue {
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingDocumentKeys(Iterable<DocumentKey> iterable) {
        ImmutableSortedSet<Integer> immutableSortedSet = new ImmutableSortedSet<>(Collections.emptyList(), new MemoryMutationQueue$$ExternalSyntheticLambda0());
    ...
    long getByteSize(LocalSerializer localSerializer) {
        Iterator<MutationBatch> it = this.queue.iterator();
        long serializedSize = 0;
        while (it.hasNext()) {
            serializedSize += (long) localSerializer.encodeMutationBatch(it.next()).getSerializedSize();
        }
