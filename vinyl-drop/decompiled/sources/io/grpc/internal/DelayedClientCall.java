...
public class DelayedClientCall<ReqT, RespT> extends ClientCall<ReqT, RespT> {
    ...
    private ScheduledFuture<?> scheduleDeadlineIfNeeded(ScheduledExecutorService scheduledExecutorService, @Nullable Deadline deadline) {
        ...
        long jTimeRemaining = deadline != null ? deadline.timeRemaining(TimeUnit.NANOSECONDS) : Long.MAX_VALUE;
        if (deadline2 != null && deadline2.timeRemaining(TimeUnit.NANOSECONDS) < jTimeRemaining) {
            jTimeRemaining = deadline2.timeRemaining(TimeUnit.NANOSECONDS);
            Logger logger2 = logger;
            if (logger2.isLoggable(Level.FINE)) {
                StringBuilder sb = new StringBuilder(String.format(Locale.US, "Call timeout set to '%d' ns, due to context deadline.", Long.valueOf(jTimeRemaining)));
                if (deadline == null) {
        ...
        long jAbs = Math.abs(jTimeRemaining) / TimeUnit.SECONDS.toNanos(1L);
        long jAbs2 = Math.abs(jTimeRemaining) % TimeUnit.SECONDS.toNanos(1L);
