    ...
    private final class BufferChainOutputStream extends OutputStream {
        ...
        public void write(byte[] bArr, int i, int i2) {
            ...
            while (i2 > 0) {
                ...
                WritableBuffer writableBuffer = this.current;
                if (iMin == 0) {
                    WritableBuffer writableBufferAllocate2 = MessageFramer.this.bufferAllocator.allocate(Math.max(i2, writableBuffer.readableBytes() * 2));
                    this.current = writableBufferAllocate2;
                    this.bufferList.add(writableBufferAllocate2);
                } else {
                    writableBuffer.write(bArr, i, iMin);
                    i += iMin;
                    i2 -= iMin;
                }
            }
