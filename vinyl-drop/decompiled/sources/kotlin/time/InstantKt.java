...
public final class InstantKt {
    ...
    private static final int[] POWERS_OF_TEN = {1, 10, 100, 1000, ModuleDescriptor.MODULE_VERSION, 100000, 1000000, DefaultEncoderProfilesProvider.DEFAULT_VIDEO_BITRATE_FHD, 100000000, 1000000000};
    ...
    static final boolean parseIso$lambda$0(char c) {
        return c == '-';
    }

    static final boolean parseIso$lambda$10(char c) {
        return '0' <= c && c < ':';
    }

    static final boolean parseIso$lambda$2(char c) {
        return c == '-';
    }

    static final boolean parseIso$lambda$4(char c) {
        return c == 'T' || c == 't';
    }

    static final boolean parseIso$lambda$6(char c) {
        return c == ':';
    }

    static final boolean parseIso$lambda$8(char c) {
        return c == ':';
    }
    ...
    public static final InstantParseResult parseIso(CharSequence charSequence) {
        ...
        if (year >= 0) {
            j = j2 + (((((long) i3) + year) / ((long) 4)) - ((((long) 99) + year) / ((long) 100))) + ((year + ((long) 399)) / ((long) LogSeverity.WARNING_VALUE));
        } else {
    ...
    public static final String formatIso(Instant instant) throws IOException {
        ...
        if (Math.abs(year) < 1000) {
            ...
            if (year >= 0) {
                Intrinsics.checkNotNullExpressionValue(sb2.append(year + ModuleDescriptor.MODULE_VERSION).deleteCharAt(0), "deleteCharAt(...)");
            } else {
                Intrinsics.checkNotNullExpressionValue(sb2.append(year - ModuleDescriptor.MODULE_VERSION).deleteCharAt(1), "deleteCharAt(...)");
            }
