...
public final class RemoteStore implements WatchChangeAggregator.TargetMetadataProvider {
    ...
    public RemoteStore(DatabaseId databaseId, final RemoteStoreCallback remoteStoreCallback, LocalStore localStore, Datastore datastore, final AsyncQueue asyncQueue, ConnectivityMonitor connectivityMonitor) {
        ...
        connectivityMonitor.addCallback(new Consumer() { // from class: com.google.firebase.firestore.remote.RemoteStore$$ExternalSyntheticLambda2
            ...
            public final void accept(Object obj) {
                this.f$0.m742lambda$new$1$comgooglefirebasefirestoreremoteRemoteStore(asyncQueue, (ConnectivityMonitor.NetworkStatus) obj);
            }
        });
    }

    /* JADX INFO: renamed from: lambda$new$1$com-google-firebase-firestore-remote-RemoteStore, reason: not valid java name */
    /* synthetic */ void m742lambda$new$1$comgooglefirebasefirestoreremoteRemoteStore(AsyncQueue asyncQueue, final ConnectivityMonitor.NetworkStatus networkStatus) {
        asyncQueue.enqueueAndForget(new Runnable() { // from class: com.google.firebase.firestore.remote.RemoteStore$$ExternalSyntheticLambda0
            ...
            public final void run() {
                this.f$0.m741lambda$new$0$comgooglefirebasefirestoreremoteRemoteStore(networkStatus);
            }
        });
    }

    /* JADX INFO: renamed from: lambda$new$0$com-google-firebase-firestore-remote-RemoteStore, reason: not valid java name */
    /* synthetic */ void m741lambda$new$0$comgooglefirebasefirestoreremoteRemoteStore(ConnectivityMonitor.NetworkStatus networkStatus) {
    ...
    private void raiseWatchSnapshot(SnapshotVersion snapshotVersion) {
        ...
        for (Map.Entry<Integer, TargetChange> entry : remoteEventCreateRemoteEvent.getTargetChanges().entrySet()) {
            ...
            if (!value.getResumeToken().isEmpty()) {
                ...
                TargetData targetData = this.listenTargets.get(Integer.valueOf(iIntValue));
                if (targetData != null) {
                    this.listenTargets.put(Integer.valueOf(iIntValue), targetData.withResumeToken(value.getResumeToken(), snapshotVersion));
                }
            }
        }
        for (Map.Entry<Integer, QueryPurpose> entry2 : remoteEventCreateRemoteEvent.getTargetMismatches().entrySet()) {
            ...
            TargetData targetData2 = this.listenTargets.get(Integer.valueOf(iIntValue2));
            if (targetData2 != null) {
                this.listenTargets.put(Integer.valueOf(iIntValue2), targetData2.withResumeToken(ByteString.EMPTY, targetData2.getSnapshotVersion()));
                sendUnwatchRequest(iIntValue2);
                sendWatchRequest(new TargetData(targetData2.getTarget(), iIntValue2, targetData2.getSequenceNumber(), entry2.getValue()));
            }
        }
    ...
    private void handleWriteError(Status status) {
        ...
        if (Datastore.isPermanentWriteError(status)) {
            ...
            this.writeStream.inhibitBackoff();
            this.remoteStoreCallback.handleRejectedWrite(mutationBatchPoll.getBatchId(), status);
            fillWritePipeline();
        }
    }
    ...
    public void executePipeline(ExecutePipelineRequest executePipelineRequest, PipelineResultObserver pipelineResultObserver) {
        if (canUseNetwork()) {
            this.datastore.executePipeline(executePipelineRequest, pipelineResultObserver);
        } else {
            pipelineResultObserver.onError(new FirebaseFirestoreException("Failed to get result from server.", FirebaseFirestoreException.Code.UNAVAILABLE));
        }
    }
}
