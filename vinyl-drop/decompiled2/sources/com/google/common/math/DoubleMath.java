...
public final class DoubleMath {
    ...
    static double roundIntermediate(double x, RoundingMode mode) {
        ...
        switch (AnonymousClass1.$SwitchMap$java$math$RoundingMode[mode.ordinal()]) {
            case 1:
                ...
                return x;
            case 2:
                return (x >= AudioStats.AUDIO_AMPLITUDE_NONE || isMathematicalInteger(x)) ? x : ((long) x) - 1;
            case 3:
                return (x <= AudioStats.AUDIO_AMPLITUDE_NONE || isMathematicalInteger(x)) ? x : ((long) x) + 1;
            case 4:
                return x;
            case 5:
                ...
                return ((long) x) + ((long) (x > AudioStats.AUDIO_AMPLITUDE_NONE ? 1 : -1));
            case 6:
                return Math.rint(x);
            case 7:
    ...
    public static boolean isPowerOfTwo(double x) {
        if (x > AudioStats.AUDIO_AMPLITUDE_NONE && DoubleUtils.isFinite(x)) {
    ...
    */
    public static int log2(double x, RoundingMode mode) {
        ...
        Preconditions.checkArgument(x > AudioStats.AUDIO_AMPLITUDE_NONE && DoubleUtils.isFinite(x), "x must be positive and finite");
    ...
    public static boolean isMathematicalInteger(double x) {
        if (DoubleUtils.isFinite(x)) {
            return x == AudioStats.AUDIO_AMPLITUDE_NONE || 52 - Long.numberOfTrailingZeros(DoubleUtils.getSignificand(x)) <= Math.getExponent(x);
        }
        return false;
    }
