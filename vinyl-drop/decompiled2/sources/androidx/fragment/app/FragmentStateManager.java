...
class FragmentStateManager {
    ...
    int computeExpectedState() {
        ...
        if (this.mFragment.mFromLayout) {
            ...
            if (z) {
                ...
                iMin = i2 < 4 ? Math.min(iMin, this.mFragment.mState) : Math.min(iMin, 1);
            }
        }
    ...
    void createView() {
        ...
        Fragment fragment2 = this.mFragment;
        if (viewGroup2 != null) {
            viewGroup = fragment2.mContainer;
        } else if (fragment2.mContainerId != 0) {
            int i = this.mFragment.mContainerId;
            Fragment fragment3 = this.mFragment;
            if (i == -1) {
                throw new IllegalArgumentException("Cannot create fragment " + fragment3 + " for a container view with no id");
            }
            viewGroup = (ViewGroup) fragment3.mFragmentManager.getContainer().onFindViewById(this.mFragment.mContainerId);
            if (viewGroup == null) {
        ...
        if (this.mFragment.mView != null) {
            ...
            if (zIsAttachedToWindow) {
                ViewCompat.requestApplyInsets(fragment5.mView);
            } else {
                final View view = fragment5.mView;
            ...
            fragmentLifecycleCallbacksDispatcher.dispatchOnFragmentViewCreated(fragment6, fragment6.mView, this.mFragment.mSavedFragmentState, false);
