...
public final class RoundedPolygon {
    ...
    public final RoundedPolygon transformed(PointTransformer f) {
        ...
        long jM421transformedso9K2fw = PointKt.m421transformedso9K2fw(FloatFloatPair.m322constructorimpl(this.centerX, this.centerY), f);
        List listCreateListBuilder = CollectionsKt.createListBuilder();
        int size = this.features.size();
        for (int i = 0; i < size; i++) {
            listCreateListBuilder.add(this.features.get(i).transformed$graphics_shapes_release(f));
        }
        return new RoundedPolygon(CollectionsKt.build(listCreateListBuilder), PointKt.m414getXDnnuFBc(jM421transformedso9K2fw), PointKt.m415getYDnnuFBc(jM421transformedso9K2fw));
    }
    ...
    public final RoundedPolygon normalized() {
        ...
        return transformed(new PointTransformer() { // from class: androidx.graphics.shapes.RoundedPolygon.normalized.1
            ...
            public final long mo423transformXgqJiTY(float f6, float f7) {
                float f8 = f6 + f4;
                float f9 = fMax;
                return FloatFloatPair.m322constructorimpl(f8 / f9, (f7 + f5) / f9);
            }
        });
    }
    ...
    public final float[] calculateMaxBounds(float[] bounds) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            long jM402pointOnCurveOOQOV4g$graphics_shapes_release = cubic.m402pointOnCurveOOQOV4g$graphics_shapes_release(0.5f);
            fMax = Math.max(fMax, Math.max(fDistanceSquared, Utils.distanceSquared(PointKt.m414getXDnnuFBc(jM402pointOnCurveOOQOV4g$graphics_shapes_release) - this.centerX, PointKt.m415getYDnnuFBc(jM402pointOnCurveOOQOV4g$graphics_shapes_release) - this.centerY)));
        }
